...
if (is_array($site_url)) {
    $current_host = fetch_or_get($_SERVER['HTTP_HOST'], 'none');
    // Remove port from current host if present for comparison
    $current_host_only = explode(':', $current_host)[0];
    
	if ($site_url['host'] != $current_host_only) {
		// cl_redirect("/");
	}
}
